...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func checkForReachability(notfication: NSNotification){
        let remoteHostStatus = self.reachability.currentReachabilityStatus()
        if remoteHostStatus.rawValue == NotReachable.rawValue{
            let alertController = UIAlertController(title: "Error", message: "Please check your Internet Connection", preferredStyle: .Alert)
            let OKAction = UIAlertAction(title: "OK", style: .Default, handler: nil)
            alertController.addAction(OKAction)
            window?.visibleViewController?.presentViewController(alertController, animated: true, completion: nil)
        }
